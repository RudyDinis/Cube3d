...
/*   Updated: 2026/02/22 20:01:45 by rdinis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	load_img(t_data *data)
{
	...
	data->minimap->door = mlx_xpm_file_to_image(data->mlx,
			"./src/img/door.xpm", &img_width, &img_height);
	if (!data->minimap->door)
		return (error_handler(data, 3), 1);
	data->minimap->door_addr = mlx_get_data_addr(
			data->minimap->door, &bits_per_pixel, &size_line, &endian);
	data->minimap->door_bpp = bits_per_pixel;
	data->minimap->door_size_line = size_line;
	return (0);
}
